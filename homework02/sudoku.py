...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    if find_empty_positions(grid) == (10, 10):
        return grid
    row = find_empty_positions(grid)[0]
    col = find_empty_positions(grid)[1]
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    yes = set('123456789')
    for i in range(0, 9):
        if (set(get_row(solution, (i, 0))) != yes) or (set(get_col(solution, (0, i))) != yes) or (set(get_block(solution, ((i // 3)*3,(i % 3)*3))) != yes):
            return False
    return True
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    True
    """
    example = [['.' for n in range(0, 9)] for k in range(0, 9)]
    new = solve(example)
    while N < 81:
